...
{
    internal class Indexer : Database, IIndexer
    {
        ...
        {
            var typeStore = await TypeStoreFactory
                .GetOrAddTypeStoreAsync(mdHead.Md)
                .ConfigureAwait(false);
            var indexer = new Indexer(/*mdHead.Md, */typeStore);

            var typeStores = await typeStore.GetAllAsync()
                .ConfigureAwait(false);
